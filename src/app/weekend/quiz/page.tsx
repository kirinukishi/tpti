...
import { calculateScores, determineWeekendBossType } from "../../../lib/wbti/scoring";

export default function WBTIQuizPage() {
  ...
  const handleSubmit = () => {
    ...
    const type = determineWeekendBossType(scores);
    router.push(`/weekend/result/${type.toLowerCase()}`);
  };
        ...
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button
            ...
            className={`px-8 py-3 rounded-xl font-bold text-sm transition-all duration-200 ${
              answeredCount === totalCount
                ? "bg-gradient-to-r from-amber-500 to-yellow-500 text-white shadow-lg hover:scale-105"
